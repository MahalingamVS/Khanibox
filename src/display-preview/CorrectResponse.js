...
// import { ScreenLabel } from "../../../common/screenLabel/ScreenLabelContainer";
import {
...
const CorrectResponse = ({ content, onUpdate }) => {
  ...
  const buildCorrectResponse = (e) => {
    ...
    const responses = content.correctResponse.responses;
    if (
      selectedText !== null &&
      ...
      for (let i = 0; i < content.foils.length; i++) {
        if (
          ...
          if (paragraphElements[i] !== undefined) {
            ...
            if (
              content.correctResponse.responses.includes(
  ...
  const correctResponseHighlight = (updatedPassageContent) => {
    ...
    if (
      ...
      for (let i = 0; i < content.foils.length; i++) {
        let classNames = "";
        if (
          content.correctResponse.responses.includes(content.foils[i].foilId)
        ) {
          classNames = getOnSelectClassNames();
        }
        classNames =
  ...
  const buildDataForHighlight = (foilId, foilText) => {
    let classNames = "";
    if (content.correctResponse.responses.includes(foilId)) {
      classNames = getOnSelectClassNames();
    }
  ...
  return (
    <div>
      <h4>
        {/* <ScreenLabel attr_code="st_correct_response" /> */}
        st_correct_response
      </h4>
      <div
